...
const config = JSON.parse(fs.readFileSync(path.join(process.cwd(), 'load-generator.json'), 'utf8'))
...
const compileTableData = () => Object.keys(tableData).reduce((acc, key) => {
...
const generateY = amount => ' '.repeat(amount).split('').map((str) => 0)
const generateX = amount => generateY(amount).map((str, i) => i.toString()).reverse()
const average = arr => arr.reduce((acc, num) => acc + num, 0) / arr.length || 1
const lineSuccess = {
  title: '[ms]',
  style: { line: 'green' },
  xLabelPadding: 0,
